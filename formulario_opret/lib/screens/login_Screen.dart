...
class _LoginScreenState extends State<LoginScreen> {
  ...
  final ApiServiceToken _serviceToken = ApiServiceToken('https://10.0.2.2:7002');
  ...
  Future<void> _login() async {
    ...
    LoginEmpleado loginEmpleado = LoginEmpleado(userName: userName, password: password);

    try{
      String? tokenAdmin = await _serviceToken.loginAdministrador(loginAdmin);
      String? tokenEmpleado = await _serviceToken.loginUser(loginEmpleado);

      if (tokenAdmin != null){
        // Decodificar el token JWT para obtener los claims
        Map<String, dynamic> decodedTokenAdmin = _parseJwt(tokenAdmin);
        String role = decodedTokenAdmin['rol'];
        ...
        if (role == 'Administrador'){
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('No se reconoce el rol'))
          );
        }

      } else if (tokenEmpleado != null) {
        Map<String, dynamic> decodedTokenEmpl = _parseJwt(tokenEmpleado);
        String role = decodedTokenEmpl['rol'];
        String userNameEmpl = decodedTokenEmpl['usuario'];
        String email = decodedTokenEmpl['email'];
        String id = decodedTokenEmpl['id'];
        String cedula = decodedTokenEmpl['cedula'];
        ...
        if (role == 'Empleado') {
          ...
          // Si el rol es Empleado, redirige a la pantalla de empleado
          Navigator.push(
            context,
            MaterialPageRoute(
          ...
          //   const SnackBar(content: Text('No se reconoce el rol'))
          // );
          _showSnackBar('No se reconoce el rol del empleado.');
        }
